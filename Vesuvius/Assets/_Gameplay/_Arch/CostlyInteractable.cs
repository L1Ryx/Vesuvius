...
{
    public class CostlyInteractable : Interactable
    {
        ...
        protected override void HandleProximityLogic()
        {
            base.HandleProximityLogic();
        ...
        private void UpdateCostUIColor()
        {
            ...
            {
                interactPromptText.text = "Not enough Clink";
            }
            //interactPromptText.text = hasEnoughCurrency ? controlsTextReplacement.OnControlsChanged() : "Not enough Clink";
        }

        protected override void OnInteractPerformed(InputAction.CallbackContext context)
        {
            // Check if the player has enough currency
            if (playerInfo != null && playerInfo.GetTotalCurrency() >= cost)
            {
                ...
                // Unlock the gate
                base.OnInteractPerformed(context);

                Debug.Log($" {cost} currency deducted. Remaining currency: {playerInfo.GetTotalCurrency()}");
            }
            else
            {
